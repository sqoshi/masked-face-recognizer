...
from config.mask_strategy import MaskingStrategy
...
def fix_rect(rect: dlib.rectangle) -> dlib.rectangle:
...
def filter_eyes_landmarks(
    landmarks: Dict[int, Tuple[int, int]]
) -> List[Tuple[int, int]]:
    """Collect eyes landmarks"""
    eyes_indexes = list(range(37, 48))
    out_arr = []
...
def filter_upper_landmarks(
    landmarks: Dict[int, Tuple[int, int]]
) -> List[Tuple[int, int]]:
    """Collect landmarks of upper face - eyes, upper nose, eyebrows"""
    eyes_indexes = list(range(37, 48))
    eyebrows_indexes = list(range(18, 27))
...
def gouge_landmarks_values(
    face_image: NDArray[Any], coordinates: List[Tuple[int, int]]
) -> NDArray[Any]:
    """Leaves only important pixels, related with given list of coords.."""
...
class FaceDetector:
    ...
    landmarks_predictor = LandmarksPredictor(
        predictor_fp=None, show_samples=False, face_detection=True, auto_download=True
    )
    ...
    def vector_generator(
        self,
        df: pd.DataFrame,
        vector_func: Callable,  # type:ignore
        landmarks_detection: bool = True,
    ) -> Generator[Tuple[NDArray[Any], Image], None, None]:
        """Creates generator which yields vectors created by openface from images."""
            ...
            if face_rectangles:
                ...
                if landmarks_detection:
                    self.landmarks_predictor.detect(
                        images_list=[(face_crop, row["filename"])], create_map=False
                    )
                    landmarks = self.landmarks_predictor.get_landmarks()
                    important_coords = filter_upper_landmarks(
                        next(iter(landmarks.values()))  # pylint:disable=stop-iteration-return
                    )
                    ...
                    if row["impose_mask"]:
                        if row["impose_mask"] == MaskingStrategy.alternately:
                            ...
                            self._last_mask = [
                                x for x in self._real_masks if x != self._last_mask
                            ][0]
