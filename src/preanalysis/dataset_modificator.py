...
class DatasetModifier:
    ...
    @staticmethod
    def modify(
            df: pd.DataFrame, mask_ratio: float = 0.0, inplace: bool = True, skip_unknown: bool = False
    ) -> pd.DataFrame:
        """
        ...
        if skip_unknown:
            new_df = new_df.drop(new_df[(new_df.identity == "unknown") | (new_df.identity is None)].index)
